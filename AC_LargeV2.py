...
absolute_path_1 = ('Negative')
file_path_1 = os.path.join(script_dir_1,absolute_path_1)
Negative_txt_files = []
for file in os.listdir(file_path_1):
    if file.endswith(".txt"):
        Negative_txt_files.append(file)

# get_positive
script_dir_2 = os.path.dirname(os.path.abspath(__file__))
absolute_path_2 = ('Positive')
file_path_2 = os.path.join(script_dir_2,absolute_path_2)
Positive_txt_files = []
for file in os.listdir(file_path_2):
    if file.endswith(".txt"):
        Positive_txt_files.append(file)
def get_file_path_back(folder_path, file_name):
...
def get_all_txt_files_all(folder_path):
    txt_files = []
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            if file.endswith(".txt"):
...
class AC_FUN_SUPER_LARGE_V2(AC_FUN,VAEEncode):
    ...
    def INPUT_TYPES(self):
        ...
        return {"required": { 
            ...
            "Switch": (self.Switch, ),
            "PositiveList":(get_all_txt_files(file_path_2),),
            "NegativeList":(Negative_txt_files,),
    ...
    @classmethod
    def negative_index(cls,NegativeList):
        new_path = os.path.join(file_path_1,NegativeList)
    ...
    @classmethod
    def get_file_path_back(cls,folder_path, file_name):
        for root, dirs, files in os.walk(folder_path):
            for file in files:
                if file == file_name:
                    return os.path.join(root, file)
    ...
    @classmethod
    def positive_index_back(cls,file_name):
        new_path = cls.get_file_path_back(file_path_2,file_name)
        f = open(new_path,'r',encoding='utf-8')
        result = f.read()
        f.close()
        return result
    ...
    def super_large(self,
            ...
            image=None,Switch=None,NegativeList=None,PositiveList=None
                    ):
        if boolean == "Single":
        ...
        # Switch
        if Switch == 'Manual operation':
            positive = self.encode(clip,positive)
            negative = self.encode(clip,negative)

        if Switch == 'Auto':
            negative = self.negative_index(NegativeList)
            negative = self.encode(clip,negative)
            
            positive = self.positive_index_back(PositiveList)
            positive = self.encode(clip,positive)
